...
1. Identify the manuscript, declared paper mode, registries, manifest, result files, figures, and citations.
...
5. Write both `paper_review_report.md` and `paper_review_report.json`, including blocking issues, unsupported claims, missing figures/tables, stale evidence, and return stage.
6. Keep subjective rubric observations separate from machine gates. Do not create a competition score from a constant or use it as quality evidence.
...
Run `scripts/execute_skill.py` with the manuscript, source problem, result object, registries, bibliography, and project manifest. Register `paper_review_report`. The runtime trace, not a hand-written review log, is the evidence that this Skill was invoked.
