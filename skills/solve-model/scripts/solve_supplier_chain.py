...
def main(root: Path) -> None:
    ...
    for name, plan in scenario_orders.items():
        for week in range(1, WEEKS + 1):
            week_plan = plan.copy()
            # A deterministic seasonal perturbation keeps all 24 schedules explicit.
            week_plan["raw_order"] *= 0.96 + 0.08 * ((week - 1) % 6) / 5
    ...
    for name, plan in scenario_orders.items():
        ...
        for week in range(1, WEEKS + 1):
            ...
            rows.append({
                ...
                "raw_received": float(sub.raw_received.sum()),
                "product_equiv_received": float(sub.product_equiv_received.sum()),
    ...
    dump(results / "model_specification.json", {
        "model": "four-question decomposition: weighted supplier importance + capacity-cover MILP + material-priority order allocation + least-loss transport assignment + bootstrap validation",
        "code_function": "solve_supplier_chain.main", "capacity_quantile": 0.75,
        "assumptions": ["one supplier is assigned to one carrier where capacity allows", "carrier capacity is 6000 raw m3/week", "planning capacity is positive-supply q75"],
    })
    dump(results / "solver_validation.json", {"status": "OPTIMAL", "q2_capacity_cover": summary["diagnostics"]["selected_capacity_after_loss"], "demand": DEMAND})

    evidence_files = {"RESULT": results / "result_object.json", "SIMULATION": results / "simulation_metrics.json"}
    ...
    claims = [
        ...
        ("CLAIM_Q2_COST", "RESULT", "/questions/q2/baseline_cost_index"), ("CLAIM_Q2_RECEIVED", "RESULT", "/questions/q2/mean_received_product"),
    ...
    formulas = []
    formulas_latex = [r"C_i=0.45\tilde q_i+0.20a_i+0.20s_i+0.15f_i", r"q_i=Q_{0.75}(S_i\mid S_i>0)", r"p_i=q_i/\gamma_i", r"\min\sum_i x_i", r"\sum_i(1-\ell^*)p_ix_i\ge D", r"\sum_j y_{ij}=o_i", r"\sum_i y_{ij}\le6000", r"R=\sum_{ij}(1-\ell_j)y_{ij}/\gamma_i", r"\max\sum_i(1-\ell^*)p_i", r"\hat\rho=\frac1{60}\sum_{b=1}^{60}\rho_b"]
    ...
    table_files = ["top50_suppliers.csv", "supplier_ranking.csv", "carrier_loss_summary.csv", "q2_weekly_plan.csv", "q3_material_mix.csv", "q4_weekly_plan.csv"]
