...
def main(root:Path)->None:
 ...
 for fid,column,title in [("fig_supplier_scores","score","Supplier consensus scores"),("fig_top50_capacity","product_equiv_capacity","Top-50 product-equivalent capacity")]:
  ...
  fig,ax=plt.subplots(figsize=(7,3.4));ax.plot(range(1,len(subset)+1),subset[column].to_numpy(),lw=1);ax.set(xlabel="rank",ylabel=column,title=title);fig.tight_layout();svg=figures/f"{fid}.svg";png=figures/f"{fid}.png";fig.savefig(svg);fig.savefig(png,dpi=250);plt.close(fig)
