...
def main(argv: list[str] | None = None, *, default_skill: str | None = None) -> int:
    ...
    try:
        ...
        contract_errors = validate_records(args.skill, start["inputs"], [{"role": role} for role, _ in outputs])
        if contract_errors:
            terminal = finish_skill_run(args.project_root, start["run_id"], status="BLOCKED", warnings=contract_errors, exit_code=1)
            print(json.dumps(terminal, ensure_ascii=False, indent=2), file=sys.stderr)
            return 1
